...
class VisionLanguageModel(nn.Module):
    ...
    def _replace_img_tokens_with_embd(self, input_ids, token_embd, image_embd):
        """
        Replace every image-token placeholder in `input_ids` with the corresponding slice from `image_embd`. 
    ...
    def forward(self, input_ids, images, attention_mask=None, targets=None):
        ...
        if isinstance(images, list):
            if not images:
                images = torch.empty(
                    0, self.cfg.vit_channels, self.cfg.vit_image_size, self.cfg.vit_image_size, 
        ...
        # replace images tokens with embed
    ...
    def generate(self, input_ids, images, attention_mask=None, max_new_tokens=5, top_k=50, top_p=0.9, temperature=0.5, greedy=False):
        ...
        # 3) Combine image and text embedding
        ...
        prefill_output, kv_cache_list = self.decoder(
            ...
            attention_mask=attention_mask,  # Use the provider attention mask
            kv_cache=None,
            start_pos=0,
        )
    ...
    def save_pretrained(self, save_directory: str) -> None:
        ...
        # Save config
        with open(save_directory.joinpath("config.json"), "w") as file:
            file.write(json.dumps(asdict(self.cfg), indent=4))
        # Save weights as safetensors
        save_model(self, save_directory.joinpath("model.safetensors"))
    ...
    def push_to_hub(self, repo_id: str, private: bool=False) -> None:
        ...
        print(f"Create repo: {repo_url}")
        
        with tempfile.TemporaryDirectory() as save_path:
            # Save to tmp directory
            self.save_pretrained(save_path)
            # Save model card
            with open(Path(save_path).joinpath("README.md"), "w") as file:
                file.write(MODEL_CARD_TEMPLATE.format(repo_id=repo_id))
            # Upload
