...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, position_ids: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Tuple of two tensors (cos, sin), 
            each of shape (batch_size, seq_len, dim), representing rotary embeddings.
        """
...
def apply_rotary_pos_embd(q: torch.Tensor, k: torch.Tensor, 
...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Use scaled dot product attention if avaliable
    ...
    def forward(self, 
        ...
        if self.sdpa and x.device.type != "mps":
            ...
            if additive_attn_mask is not None:
                # additive_attn_mask is [B, 1, 1, T_kv], needs to be broadcast to [B, n_head, T_curr, T_kv]
...
class LanguageModelBlock(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.norm2 = RMSNorm(cfg)  # Post Attenion Norm
...
class LanguageModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor=None, kv_cache: List[Dict]=None, start_pos: int=0):
        ...
        cos, sin = self.rotary_embd(current_position_ids)  # Get rotary position embedding for current tokens
    ...
    def generate(self, inputs: torch.Tensor, max_new_tokens: int=20):
        ...
        for i in range(max_new_tokens):
            if self.lm_use_tokens:
                # New the model outputs logits
                next_output = torch.argmax(last_output, dim=2, keepdim=True)
            else:
